...
#if DT_NODE_EXISTS(bosch_bme280)
...
int main() {
  ...
  while (1) {
    ...
    decoder->decode(buf, (struct sensor_chan_spec){SENSOR_CHAN_AMBIENT_TEMP, 0}, &temp_fit, 1,
                    &temp_data);
    ...
    decoder->decode(buf, (struct sensor_chan_spec){SENSOR_CHAN_PRESS, 0}, &press_fit, 1,
                    &press_data);

    uint32_t hum_fit = 0;
    struct sensor_q31_data hum_data = {0};

    decoder->decode(buf, (struct sensor_chan_spec){SENSOR_CHAN_HUMIDITY, 0}, &hum_fit, 1,
                    &hum_data);
    ...
    printk("temp: %s%d.%d; press: %s%d.%d; humidity: %s%d.%d, elements in "
           ...
           PRIq_arg(hum_data.readings[0].humidity, 6, hum_data.shift), temp_buffer.size());
